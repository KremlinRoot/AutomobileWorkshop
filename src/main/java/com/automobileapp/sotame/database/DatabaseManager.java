...
public class DatabaseManager {
    ...
    public static List<BudgetOrder> getAllBudgetOrders() {
        ...
        String query = "SELECT o.idOrder, o.orderNumber, o.estimatedCompletionDate, o.workDescription, o.totalCost, " +
                "b.idBudgetOrder, b.laborHours, b.partCost " +
                "FROM ORDER_WORK o " +
        ...
        try (Connection conn = getConnection();
            ...
            while (rs.next()) {
                ...
                BudgetOrder budgetOrder = new BudgetOrder(rs.getInt("idBudgetOrder"),
                        order,
    ...
    public static void updateBudgetOrder(BudgetOrder budgetOrder) throws SQLException {
        String query = "UPDATE budget_order SET laborHours = ?, partCost = ?, idOrder = ?  WHERE idBudgetOrder = ? ";

        try (Connection connection = getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(query)) {

//            preparedStatement.setInt(1, budgetOrder.getOrder().getIdOrder());
            preparedStatement.setDouble(1, budgetOrder.getLaborHours());
            preparedStatement.setDouble(2, budgetOrder.getPartCost());
            preparedStatement.setInt(3, budgetOrder.getOrder().getIdOrder());
            preparedStatement.setInt(4, budgetOrder.getIdBudgetOrder());// Update based on ID
            int rowAffected = preparedStatement.executeUpdate();
            if (rowAffected == 0) {
                System.out.println("No se actualizo ninguna fila con idBudgetOrder:" + budgetOrder.getIdBudgetOrder());
            } else {
                System.out.println("Se actualizo el presupuesto con idBudgetOrder: " + budgetOrder.getIdBudgetOrder());
